...
def validate_namespace_exists(namespace_id: str, memory_bank, raise_error: bool = True) -> bool:
    ...
    Args:
        namespace_id: ID of the namespace to check
    ...
    # Check cache first
    if namespace_id in _namespace_cache:
        exists = _namespace_cache[namespace_id]
    ...
    try:
        ...
        if hasattr(memory_bank, "namespace_exists"):
            exists = memory_bank.namespace_exists(namespace_id)
        else:
            # Fall back to direct query via dolt_reader
            query = "SELECT id FROM namespaces WHERE id = %s"
            result = memory_bank.dolt_reader._execute_query(query, (namespace_id,))
            exists = len(result) > 0

        # Cache result
        _namespace_cache[namespace_id] = exists
...
def validate_namespaces_exist(
    ...
    Args:
        namespace_ids: Set of namespace IDs to verify
        memory_bank: Memory bank interface to use for verification
        raise_error: Whether to raise an exception if any namespace doesn't exist

    Returns:
        Dictionary mapping namespace IDs to existence status
    ...
    for namespace_id in namespace_ids:
        ...
        # Check cache
        if namespace_id in _namespace_cache:
            result[namespace_id] = _namespace_cache[namespace_id]
            if not result[namespace_id]:
                missing_namespaces.append(namespace_id)
        else:
            remaining_ids.add(namespace_id)

    # If there are any remaining IDs not in cache, check them
    if remaining_ids:
        try:
            # Build bulk query for remaining namespaces
            placeholders = ", ".join(["%s"] * len(remaining_ids))
            query = f"SELECT id FROM namespaces WHERE id IN ({placeholders})"
            existing_results = memory_bank.dolt_reader._execute_query(query, tuple(remaining_ids))

            # Extract existing namespace IDs
            existing_ids = {row["id"] for row in existing_results}

            # Update results and cache
            for namespace_id in remaining_ids:
                exists = namespace_id in existing_ids
                result[namespace_id] = exists
                _namespace_cache[namespace_id] = exists
                if not exists:
                    missing_namespaces.append(namespace_id)
