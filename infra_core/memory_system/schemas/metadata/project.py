...
class ProjectMetadata(BaseMetadata):
    ...
    status: Literal["planning", "in-progress", "completed", "archived"] = Field(
        "planning", description="Current status of the project"
    )
    epic: Optional[str] = Field(
        None, description="Reference to an epic or larger initiative this project belongs to"
    )
    ...
    deadline: Optional[datetime] = Field(None, description="Target completion date for the project")
            ...
            "examples": [
                {
                    ...
                    "design_decisions": {
                        "dolt_llamaindex_sync": "Writes to Dolt will trigger updates"
                    },
                }
            ]
        }
