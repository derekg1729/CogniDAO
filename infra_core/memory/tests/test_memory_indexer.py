...
class TestMemoryIndexer:
    ...
    def test_indexer_creates_chroma_collection(self, test_environment):
        ...
        class MockLogseqParser:
            ...
            def extract_all_blocks(self):
                ...
                if "#thought" in self.target_tags:
                    blocks.extend([
                        {"id": "block-1", "text": "Test block with #thought tag", "tags": ["#thought"], "source_file": "test1.md"},
                        {"id": "block-3", "text": "Another #thought with some context", "tags": ["#thought"], "source_file": "test1.md"}
                    ])
                if "#broadcast" in self.target_tags:
                    blocks.extend([
                        {"id": "block-4", "text": "Test block with #broadcast tag", "tags": ["#broadcast"], "source_file": "test2.md"},
                        {"id": "block-5", "text": "Test block with #broadcast and #approved tags", "tags": ["#broadcast", "#approved"], "source_file": "test2.md"}
                    ])
                return blocks
        ...
        mock_parser_module.load_md_files = lambda folder: [os.path.join(folder, f) for f in os.listdir(folder) if f.endswith(".md")]
        mock_parser_module.extract_blocks = lambda file_path, target_tags=None: []
        sys.modules["infra_core.memory.parser"] = mock_parser_module
        ...
        try:
            ...
            # Clean up
            del sys.modules["infra_core.memory.parser"]
