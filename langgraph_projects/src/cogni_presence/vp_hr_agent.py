"""
VP HR Agent - HR specialist using LangGraph's create_react_agent.
"""
...
from .prompts import VP_HR_PROMPT

logger = get_logger(__name__)


async def create_vp_hr_node():
    """Create VP HR agent using LangGraph's create_react_agent."""
    ...
    prompt = VP_HR_PROMPT.partial(
        tool_specs=tool_specs
    )
    
    # Create and return LangGraph react agent
    model = ChatOpenAI(model_name='gpt-4o-mini')
    return create_react_agent(model=model, tools=tools, prompt=prompt, name="vp_hr")
