...
from .prompts import VP_FINANCE_PROMPT

logger = get_logger(__name__)


async def create_vp_finance_node():
    """Create VP Finance agent using LangGraph's create_react_agent."""
    ...
    prompt = VP_FINANCE_PROMPT.partial(
        tool_specs=tool_specs
    )
    
    # Create and return LangGraph react agent
    model = ChatOpenAI(model_name='gpt-4o-mini')
    return create_react_agent(model=model, tools=tools, prompt=prompt, name="vp_finance")
