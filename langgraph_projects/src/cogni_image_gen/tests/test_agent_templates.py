"""
Tests for cogni_image_gen agent template integration.
"""
...
from src.cogni_image_gen.agent import create_agent_node


class TestCogniImageGenTemplates:
    """Test the cogni_image_gen agent template integration."""
    ...
    async def test_agent_uses_template_with_tools(self):
        ...
        with patch('src.shared_utils.tool_registry.get_tools', return_value=[mock_tool]), \
             patch('src.cogni_image_gen.agent.ChatOpenAI', return_value=mock_model):
    ...
    async def test_agent_handles_template_rendering_error(self):
        ...
        with patch('src.shared_utils.tool_registry.get_tools', return_value=[mock_tool]), \
             patch('src.cogni_image_gen.agent.ChatOpenAI', return_value=mock_model), \
             patch('src.cogni_image_gen.agent.generate_tool_specs_from_mcp_tools', 
    ...
    async def test_agent_uses_default_model_name(self):
        ...
        with patch('src.shared_utils.tool_registry.get_tools', return_value=[mock_tool]), \
             patch('src.cogni_image_gen.agent.ChatOpenAI', return_value=mock_model) as mock_get_model:
