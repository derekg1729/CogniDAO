"""
Tests for layered_cogni_agent agent template integration.
"""
...
from src.layered_cogni_agent.agent import create_agent_node


class TestSimpleCogniAgentTemplates:
    """Test the layered_cogni_agent agent template integration."""
    ...
    async def test_agent_uses_template_with_tools(self):
        ...
        with patch('src.shared_utils.tool_registry.get_tools', return_value=[mock_tool]), \
             patch('src.layered_cogni_agent.agent.ChatOpenAI', return_value=mock_model):
    ...
    async def test_agent_handles_template_rendering_error(self):
        ...
        with patch('src.shared_utils.tool_registry.get_tools', return_value=[mock_tool]), \
             patch('src.layered_cogni_agent.agent.ChatOpenAI', return_value=mock_model), \
             patch('src.layered_cogni_agent.agent.generate_tool_specs_from_mcp_tools', 
    ...
    async def test_agent_uses_default_model_name(self):
        ...
        with patch('src.shared_utils.tool_registry.get_tools', return_value=[mock_tool]), \
             patch('src.layered_cogni_agent.agent.ChatOpenAI', return_value=mock_model) as mock_get_model:
