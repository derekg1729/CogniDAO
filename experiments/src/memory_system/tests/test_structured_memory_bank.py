...
from experiments.src.memory_system.structured_memory_bank import (
    StructuredMemoryBank,
    diff_memory_blocks,
)
from experiments.src.memory_system.schemas.memory_block import (
    MemoryBlock,
    BlockLink,
    ConfidenceScore,
)
...
def mock_dolt_writer():
    ...
    with (
        patch("experiments.src.memory_system.structured_memory_bank.Dolt", return_value=mock_repo),
        patch(
            "experiments.src.memory_system.structured_memory_bank.write_memory_block_to_dolt"
        ) as mock_write,
        patch(
            "experiments.src.memory_system.structured_memory_bank.commit_working_changes"
        ) as mock_commit,
        patch(
            "experiments.src.memory_system.structured_memory_bank.discard_working_changes"
        ) as mock_discard,
    ):
        # Configure the mocks
...
def mock_dolt_reader():
    ...
    with patch(
        "experiments.src.memory_system.structured_memory_bank.read_memory_block"
    ) as mock_read:
...
def mock_schema_manager():
    ...
    with patch(
        "experiments.src.memory_system.structured_memory_bank.get_schema"
    ) as mock_get_schema:
...
def memory_bank(mock_llama_memory, mock_dolt_writer, mock_dolt_reader, mock_schema_manager):
    ...
    with patch("experiments.src.memory_system.structured_memory_bank.Dolt", return_value=mock_repo):
        bank = StructuredMemoryBank(
...
class TestStructuredMemoryBank:
    ...
    def test_update_memory_block_validation_fails(
        ...
        with patch(
            "experiments.src.memory_system.structured_memory_bank.MemoryBlock.model_validate"
        ) as mock_validate:
    ...
    def test_repo_connection_reuse(self):
        ...
        with (
            patch("experiments.src.memory_system.structured_memory_bank.Dolt") as mock_dolt,
    ...
    def test_atomic_operations_llama_failure(self):
        ...
        # Setup mock objects
        with (
            patch("experiments.src.memory_system.structured_memory_bank.Dolt"),
            patch("experiments.src.memory_system.structured_memory_bank.LlamaMemory") as mock_llama,
            patch(
                "experiments.src.memory_system.structured_memory_bank.write_memory_block_to_dolt",
