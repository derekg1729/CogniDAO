...
def create_test_blocks():
    ...
    blocks.append(MemoryBlock(
        ...
        links=[
            BlockLink(to_id="test-block-1", relation="related_to")
        ],
...
@patch('experiments.scripts.sync_dolt_to_llamaindex.read_memory_blocks')
@patch('experiments.scripts.sync_dolt_to_llamaindex.LlamaMemory')
class TestSyncDoltToLlamaIndexUnit(unittest.TestCase):
    ...
    def test_successful_sync(self, mock_llama_memory_class, mock_read_blocks):
        ...
        result = sync_dolt_to_llamaindex(
            dolt_db_path=self.dolt_dir,
            llama_storage_path=self.llama_dir
        )
        ...
        self.assertEqual(mock_memory.add_block.call_count, 3,
        ...
        for i, block in enumerate(self.test_blocks):
            ...
            self.assertEqual(args[0].id, block.id,
                           f"Block at position {i} should have ID {block.id}")
    ...
    def test_block_indexing_error(self, mock_llama_memory_class, mock_read_blocks):
        ...
        self.assertEqual(mock_memory.add_block.call_count, 3,
                        "Should try to add all blocks even after error")
