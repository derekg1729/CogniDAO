...
import pytest
from memory_system.schemas.common import BlockLink, RelationType
from typing import get_args


def test_block_link_creation_with_defaults():
    """Test creating a BlockLink with minimal required fields."""
    link = BlockLink(to_id="target-block-123", relation="related_to")
...
def test_block_link_priority_validation():
    ...
    # Test valid priority
    link = BlockLink(to_id="target-123", relation="related_to", priority=0)
    assert link.priority == 0

    link = BlockLink(to_id="target-123", relation="related_to", priority=100)
    assert link.priority == 100

    # Test invalid priority
    with pytest.raises(ValueError, match="Priority must be non-negative"):
        BlockLink(to_id="target-123", relation="related_to", priority=-1)
...
def test_block_link_relation_validation():
    ...
    for relation in valid_relations:
        link = BlockLink(to_id="target-123", relation=relation)
        assert link.relation == relation

    # Test invalid relation
    with pytest.raises(ValueError):
        BlockLink(to_id="target-123", relation="invalid_relation")
...
def test_block_link_metadata_validation():
    ...
    link = BlockLink(to_id="target-123", relation="related_to", link_metadata=valid_metadata)
    assert link.link_metadata == valid_metadata

    # Test empty metadata
    link = BlockLink(to_id="target-123", relation="related_to", link_metadata={})
...
def test_block_link_created_at_auto_generation():
    ...
    before = datetime.now()
    link = BlockLink(to_id="target-123", relation="related_to")
    after = datetime.now()
