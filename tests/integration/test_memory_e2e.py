...
This makes it ideal for implementing Cogni's cognitive capabilities.
    ...
    def test_memory_e2e_flow(self, test_memory_dirs, real_content_files, caplog):
        ...
        if verbose:
            ...
            # Also enable legacy_logseq logs
        ...
        chroma_client = chromadb.PersistentClient(
            ...
            settings=chromadb.Settings(
                anonymized_telemetry=False, allow_reset=True, chroma_server_ssl_enabled=False
            ),
        )
        ...
        from legacy_logseq.memory.storage import ChromaStorage, ArchiveStorage
        ...
        if isinstance(real_content_files, list) and all(
            isinstance(item, str) for item in real_content_files
        ):
            # Real content files
        ...
        if any(
            os.path.basename(f) == "cogni_graph.md"
            for f in real_content_files
            if isinstance(f, str)
        ):
            ...
            test_queries = [
                {"query": "What is Cogni's purpose?", "description": "general query about Cogni"}
            ]
        ...
        for test_case in test_queries:
            ...
            # Perform query
            results = client.query(query_text=test_case["query"], n_results=1)
        ...
        assert after_archive_count["hot_storage"] == block_count["hot_storage"] - 1, (
            "Hot storage count did not decrease"
        )
        ...
        archive_results = client.query(
            query_text="important concepts", n_results=2, include_archived=True
        )
                ...
                for j in range(i + 1, len(memory_blocks)):
                    if memory_blocks[i].embedding and memory_blocks[j].embedding:
                        sim = cosine_similarity(
                            memory_blocks[i].embedding, memory_blocks[j].embedding
                        )
                        logger.info(
                            f"Similarity between '{memory_blocks[i].source_file}' and '{memory_blocks[j].source_file}': {sim:.4f}"
                        )
