...
try:
    ...
    sys.path.insert(0, os.path.abspath(os.path.join('{self.project_root}', 'legacy_logseq/flows/gitcogni/../..')))
...
for k in list(sys.modules.keys()):
    if 'cogni_agents' in k or 'legacy_logseq' in k:
        del sys.modules[k]
...
# Test 3: Try importing both ways from root with legacy_logseq/flows/gitcogni as working directory
os.chdir(os.path.join('{self.project_root}', 'legacy_logseq/flows/gitcogni'))
...
""")
        ...
        try:
            ...
            self.assertEqual(
                output.get("flow_import_in_prefect"),
                "success",
            ...
            self.assertEqual(
                output.get("agent_import_in_prefect"),
                "success",
