...
from infra_core.flows.broadcast.x_posting_flow import (
    get_approved_posts,
    post_to_x,
    x_posting_flow
)
...
def test_get_approved_posts(setup_test_queue, limit, expected_count):
    ...
    with patch('infra_core.tools.broadcast_queue_fetch_tool.BROADCAST_QUEUE_ROOT', BROADCAST_QUEUE_TEST_ROOT):
        with patch('infra_core.tools.broadcast_queue_fetch_tool.BROADCAST_QUEUE_SESSION', BROADCAST_QUEUE_TEST_SESSION):
            # Mock logger
            logger = MagicMock()
...
def test_x_posting_flow(setup_test_queue):
    ...
    with patch('infra_core.flows.broadcast.x_posting_flow.XChannelAdapter', return_value=MockXChannel(simulation_mode=True)):
        with patch('infra_core.flows.broadcast.x_posting_flow.get_approved_posts') as mock_get_posts:
            with patch('infra_core.flows.broadcast.x_posting_flow.post_to_x') as mock_post:
                # Setup mock data
