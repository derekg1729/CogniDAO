...
"""
    ...
    log_entry = {
        ...
        "queue_id": TEST_QUEUE_ID,
        "content_preview": state_data['content'][:50] + "..." if len(state_data['content']) > 50 else state_data['content'],
        "status": "pending"
    }
...
def custom_update_tool():
    ...
    def _update_broadcast_queue_status(scan_all=False):
        ...
        with patch('infra_core.tools.broadcast_queue_update_tool.BROADCAST_QUEUE_SESSION', BROADCAST_QUEUE_TEST_SESSION), \
...
class TestBroadcastQueueUpdate:
    ...
    def test_no_changes_returns_success_with_no_updates(self, setup_test_queue_env, create_test_queue_item, custom_update_tool):
    ...
    def test_approval_updates_status_and_logs_changes(self, setup_test_queue_env, create_test_queue_item, custom_update_tool):
        ...
        approved_content = original_content.replace("- [ ] Approved for broadcast", "- [x] Approved for broadcast")
    ...
    def test_needs_revision_updates_status(self, setup_test_queue_env, create_test_queue_item, custom_update_tool):
    ...
    def test_scan_all_processes_non_pending_items(self, setup_test_queue_env, create_test_queue_item, custom_update_tool):
        ...
        assert result["status"] == "success"
        test_updates = [update for update in result["updates"] if update["queue_id"] == TEST_QUEUE_ID]
    ...
    def test_error_handling(self, setup_test_queue_env):
        ...
        with patch('infra_core.tools.broadcast_queue_update_tool.BROADCAST_QUEUE_ROOT', Path("/nonexistent/path")):
