...
class TestPRDataProcessing(unittest.TestCase):
    ...
    def test_monitor_token_usage(self):
        ...
        long_string = (
            "This is a longer test string with more content to test token monitoring " * 10
        )
    ...
    def test_prepare_pr_data(self):
        ...
        branch_info = {"source_branch": "feature-branch", "target_branch": "main", "success": True}
        ...
        with patch("legacy_logseq.cogni_agents.git_cogni.git_cogni.datetime") as mock_datetime:
            mock_now = MagicMock()
    ...
    def test_prepare_input(self):
        ...
        with (
            patch.object(self.agent, "parse_pr_url") as mock_parse,
            ...
            mock_prepare_data.assert_called_once_with(
                mock_pr_info, mock_branch_info, mock_commit_info
            )
