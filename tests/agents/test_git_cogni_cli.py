...
from legacy_logseq.cogni_agents.git_cogni.cli import main
...
class TestGitCogniCLI(unittest.TestCase):
    ...
    def test_no_url_provided(self, mock_print, mock_exit):
        ...
        with patch("legacy_logseq.cogni_agents.git_cogni.cli.GitCogniAgent"):
            main()
        ...
        self.assertTrue(
            any(
                "Usage: git-cogni <github-pr-url>" in call[0][0]
    ...
    @patch("legacy_logseq.cogni_agents.git_cogni.cli.GitCogniAgent")
    ...
    def test_pr_review_success(self, mock_agent_class):
        ...
        with patch("sys.stdout", new=StringIO()) as fake_stdout:
            with patch("sys.exit") as mock_exit:  # noqa: F841
                ...
                mock_agent.review_and_save.assert_called_once_with(
                    "https://github.com/owner/repo/pull/123", test_mode=False
                )
    ...
    @patch("legacy_logseq.cogni_agents.git_cogni.cli.GitCogniAgent")
    @patch("sys.argv", ["git-cogni", "https://github.com/owner/repo/pull/123"])
    @patch("legacy_logseq.cogni_agents.git_cogni.cli.print")  # Patch print directly
    ...
    def test_test_mode(self, mock_agent_class):
        ...
        with patch("sys.stdout", new=StringIO()) as fake_stdout:
            with patch("sys.exit") as mock_exit:  # noqa: F841
                ...
                mock_agent.review_and_save.assert_called_once_with(
                    "https://github.com/owner/repo/pull/123", test_mode=True
                )
