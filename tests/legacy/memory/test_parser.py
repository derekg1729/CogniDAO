...
class TestLogseqParser:
    ...
    def test_standard_markdown_extraction(self, create_test_file, tmp_path):
        ...
        header_blocks = [
            b
            for b in blocks
            if "Main Header" in b["text"]
            or "Subheader" in b["text"]
            or "Second Main Header" in b["text"]
        ]
        ...
        paragraph_blocks = [
            b
            for b in blocks
            if "#important" in b["text"] or "different content" in b["text"] or "#tags" in b["text"]
        ]
    ...
    def test_parse_without_conversion_to_logseq(self, create_test_file, tmp_path):
        ...
        with open(test_file, "r", encoding="utf-8") as f:
            ...
            non_bullet_lines = (
                total_lines - bullet_lines - sum(1 for line in lines if not line.strip())
            )
        ...
        header_blocks = [
            b
            for b in blocks
            if "Cogni Documentation" in b["text"]
            or "Features" in b["text"]
            or "Implementation" in b["text"]
        ]
        paragraph_blocks = [
            b
            for b in blocks
            if "regular paragraph" in b["text"]
            or "list of features" in b["text"]
            or "multiple components" in b["text"]
        ]
