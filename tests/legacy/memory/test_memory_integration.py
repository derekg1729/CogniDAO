...
from legacy_logseq.memory.memory_tool import memory_tool
from legacy_logseq.memory.schema import MemoryBlock
from legacy_logseq.memory.storage import ChromaStorage, ArchiveStorage, CombinedStorage
...
class TestMemoryIntegration:
    ...
    def test_environment(self):
        ...
        try:
            ...
            yield {"root": test_root, "logseq_dir": logseq_dir, "output_dir": output_dir}
        finally:
    ...
    def test_complete_memory_pipeline(self, test_environment):
        ...
        combined_storage = CombinedStorage(vector_db_dir=chroma_path, archive_dir=archive_path)
        ...
        memory_client = CogniMemoryClient(
            chroma_path=chroma_path, archive_path=archive_path, collection_name=collection_name
        )
        ...
        assert len(thought_results.blocks) >= 3, (
            f"Expected >=3 thought blocks, got {len(thought_results.blocks)}"
        )
        ...
        tool_results = memory_tool(input_text="test", n_results=2, chroma_path=chroma_path)
        ...
        with patch(
            "legacy_logseq.memory.memory_indexer.init_embedding_function",
    ...
    def test_index_from_logseq_e2e(self, test_environment):
        ...
        memory_client = CogniMemoryClient(
            chroma_path=chroma_path, archive_path=archive_path, collection_name="cogni-memory-e2e"
        )
        ...
        with pytest.MonkeyPatch().context() as mp:
            ...
            mp.setattr(
                "legacy_logseq.memory.memory_indexer.init_embedding_function",
                lambda model_name: mock_embedder,
            )
        ...
        for block in broadcast_results.blocks:
            assert "#broadcast" in block.tags, (
                f"Block has tags {block.tags} but missing #broadcast tag"
            )
