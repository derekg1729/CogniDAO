...
from infra_core.cogni_api import app, send_message, verify_auth # Import verify_auth
...
class TestCogniAPI(unittest.TestCase):
    ...
    @patch('infra_core.cogni_api.send_message') # Simpler patch
    ...
    def test_chat_endpoint_missing_message(self):
        ...
        message_error_found = any(err.get('loc') == ['body', 'message'] and err.get('type') == 'missing' for err in error_details)
        self.assertTrue(message_error_found, "Validation error for missing 'message' field not found")
...
class TestSendMessageFunction(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('infra_core.cogni_api.ChatOpenAI', new=MockChatOpenAI) # Replace ChatOpenAI with our mock
    @patch('infra_core.cogni_api.AsyncIteratorCallbackHandler') # Mock the callback handler
    async def test_send_message_constructs_correct_lc_messages_no_history(self, MockCallback, *args):
    ...
    @patch('infra_core.cogni_api.ChatOpenAI', new=MockChatOpenAI)
    @patch('infra_core.cogni_api.AsyncIteratorCallbackHandler')
    async def test_send_message_constructs_correct_lc_messages_with_history(self, MockCallback, *args):
