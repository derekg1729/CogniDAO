"""
Integration tests for the MCP server with real workflow scenarios.

These tests validate the complete flow of creating, retrieving, and updating
memory blocks through the MCP server interface.
"""
...
@pytest.mark.asyncio
async def test_complete_workflow_task_lifecycle():
...
@pytest.mark.asyncio
async def test_memory_block_update_workflow():
...
@pytest.mark.asyncio
async def test_work_item_status_progression():
