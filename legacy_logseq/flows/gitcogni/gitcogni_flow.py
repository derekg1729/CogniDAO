...
def gitcogni_review_flow(pr_url=None, test_mode=False):
    ...
    agent_root = (
        project_root / "legacy_logseq" / "cogni_agents" / "git_cogni"
    )  # Define agent_root relative to project root
    ...
    logger.info(
        f"Initialized memory for project 'flows/{flow_project_name}', session '{effective_session_id}' at {session_path}"
    )
    ...
    try:
        ...
        if "commit_reviews" in review_results:
            ...
            if "final_verdict" in review_results:
                verdict_summary = (
                    review_results["final_verdict"][:200] + "..."
                    if len(review_results["final_verdict"]) > 200
                    else review_results["final_verdict"]
                )

                # Use verdict_decision field if available
                decision = review_results.get(
                    "verdict_decision", agent.get_verdict_from_text(review_results["final_verdict"])
                )
                logger.info(
                    f"VERDICT SUMMARY: {json.dumps({'verdict': verdict_summary, 'decision': decision}, indent=2)}"
                )

        # Success!
        logger.info("PR review completed successfully")
        message = (
            f"PR reviewed. Details: {review_results.get('review_file', 'See reviews directory')}"
        )
...
if __name__ == "__main__":
    ...
    if review_results:
        print(
            f"Review completed with {len(review_results.get('commit_reviews', []))} commits analyzed"
        )
