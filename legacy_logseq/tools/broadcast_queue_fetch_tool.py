...
def fetch_from_broadcast_queue(
    status: Annotated[
        str, "Status to filter by ('approved', 'pending', 'needs_revision', 'posted')"
    ],
    limit: Annotated[Optional[int], "Maximum number of items to return (0 for all)"] = 5,
    sort_by: Annotated[
        Optional[str], "Sort field ('priority', 'creation_time', 'scheduled_time')"
    ] = "priority",
    ...
    try:
        ...
        if not state_dir.exists():
            return json.dumps(
                {"status": "error", "message": f"State directory not found: {state_dir}"}, indent=2
            )
...
def update_broadcast_queue_item(
    ...
    new_status: Annotated[
        str, "New status for the item ('approved', 'pending', 'needs_revision', 'posted')"
    ],
    post_info: Annotated[
        Optional[Dict[str, Any]], "Information about the post to store (for 'posted' status)"
    ] = None,
) -> str:
    """
    ...
    try:
        ...
        if not state_file.exists():
            return json.dumps(
                {"status": "error", "message": f"State file not found for queue ID: {queue_id}"},
                indent=2,
            )
