...
# Import our schemas
from legacy_logseq.models import CompleteQueryRequest, ErrorResponse
...
MEMORY_CHROMA_PATH = "legacy_logseq/memory/chroma"
MEMORY_ARCHIVE_PATH = "legacy_logseq/memory/archive"  # Needs to exist for client
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully indexed {indexed_count} blocks from {API_INDEXED_FILES_DIR}."
            )
        except FileNotFoundError:
            logger.error(
                f" Indexed files directory not found: {API_INDEXED_FILES_DIR}. Skipping indexing."
            )
