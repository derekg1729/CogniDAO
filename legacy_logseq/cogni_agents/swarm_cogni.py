...
from legacy_logseq.tools.format_as_json_tool import format_as_json_tool, format_as_json
from legacy_logseq.tools.broadcast_queue_tool import (
    add_to_broadcast_queue_tool,
    add_to_broadcast_queue,
)
...
class CogniSwarmAgent(CogniAgent):
    def __init__(
        ...
        super().__init__(
            name=name,
            spirit_path=spirit_path or Path("legacy_logseq/cogni_spirit/spirits/swarm-cogni.md"),
    ...
    def _build_swarm(self):
        ...
        tool_agent_llm_config = {
            ...
            "cache_seed": 42,
            "timeout": 120,
            "functions": [format_as_json_tool.schema, add_to_broadcast_queue_tool.schema],
        }
    ...
    async def a_act(self, prepared_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        chat_result = await self.swarm["user_proxy"].a_initiate_chat(
            self.swarm["manager"], message=thought
        )
        ...
        result = {"output": final_output, "raw_result": all_messages, "thought_content": thought}
