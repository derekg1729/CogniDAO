...
class GitCogniAgent(CogniAgent):
    ...
    def __init__(self, agent_root: Path, external_logger=None, **kwargs):
        ...
        super().__init__(
            name="git-cogni",
            spirit_path=Path("legacy_logseq/cogni_spirit/spirits/git-cogni.md"),
    ...
    def setup_logging(verbose=False):
        ...
        agent_logger = logging.getLogger("legacy_logseq.cogni_agents.git_cogni.git_cogni")
    ...
    def monitor_token_usage(self, operation, data):
        ...
        if isinstance(data, str):
            ...
            self.logger.info(
                f"TOKEN MONITOR | {operation} | ~{word_count} words, {char_length} chars"
            )
    ...
    def parse_pr_url(self, pr_url):
        ...
        result = {"owner": None, "repo": None, "number": None, "success": False, "error": None}
    ...
    def get_pr_branches(self, pr_info):
        ...
        """
        # Initialize result
        result = {"source_branch": None, "target_branch": None, "success": False, "error": None}
    ...
    def get_pr_commits(self, pr_info):
        ...
        """
        # Initialize result
        result = {"commits": [], "success": False, "error": None}
        ...
        try:
            ...
            for commit in commits:
                ...
                total_diff_length = sum(
                    len(file.get("patch", "")) for file in files if "patch" in file
                )
    ...
    def review_pr(self, git_cogni_context, core_context, pr_data):
        ...
        for i, commit in enumerate(commits):
            self.logger.info(
                f"Reviewing commit {i + 1}/{len(commits)}: {commit['short_sha']} - {commit['message']}"
            )
            ...
            for file in commit["files"]:
                ...
                if "patch" in file_copy:
                    file_copy["patch"] = file_copy["patch"][
                        :500
                    ]  # Limit patch size to 500 chars (reduced from 1500)
            ...
            self.logger.info(
                f"COMMIT REVIEW ({commit['short_sha']}): {json.dumps({'review': commit_review}, indent=2)}"
            )
    ...
    def act(self, prepared_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        task_description = (
            f"Reviewing PR #{pr_info['number']} in {pr_info['owner']}/{pr_info['repo']}"
        )

        # Get guide using the base class method
        git_cogni_context = self.get_guide_for_task(task=task_description, guides=["git-cogni"])
