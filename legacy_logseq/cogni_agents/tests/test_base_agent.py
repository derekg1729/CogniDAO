...
from legacy_logseq.memory.memory_bank import (
    FileMemoryBank,
)  # Needed for type hinting and patching targets
...
def core_files(temp_dir):
    ...
    spirits_dir = temp_dir / "legacy_logseq" / "cogni_spirit" / "spirits"
...
def memory_instance(memory_bank_root):
    ...
    return FileMemoryBank(
        memory_bank_root=memory_bank_root, project_name="test_project", session_id="test_session"
    )
...
def test_load_spirit_from_memory(mock_memory_instance, agent_root, spirit_file, temp_dir):
    ...
    mock_memory_instance.load_or_seed_file.assert_called_once_with(
        file_name=expected_bank_filename, fallback_path=temp_dir / spirit_file
    )
...
def test_load_core_context_from_memory(
    memory_instance, agent_root, spirit_file, core_files, temp_dir
):
    ...
    memory_instance.write_context(
        "MANIFESTO.md", core_files["manifesto"].read_text(), is_json=False
    )
    memory_instance.write_context(
        "guide_cogni-core-spirit.md", core_files["core_spirit"].read_text(), is_json=False
    )
...
def test_load_core_context_mixed(
    mock_memory_instance, agent_root, spirit_file, core_files, temp_dir
):
    ...
    with patch("legacy_logseq.cogni_agents.base.FileMemoryBank", return_value=mock_memory_instance):
        with patch.object(DummyAgent, "load_spirit"):
            agent = DummyAgent(
...
def test_get_guide_for_task_default(
    ...
    with patch.object(DummyAgent, "load_spirit"):
        with patch.object(DummyAgent, "load_core_context"):
            ...
            with patch(
                "legacy_logseq.cogni_agents.base.FileMemoryBank", return_value=mock_memory_instance
            ):
                agent = DummyAgent(
...
def test_get_guide_for_task_custom(
    mock_memory_instance, agent_root, spirit_file, core_files, temp_dir
):
        ...
        with patch.object(DummyAgent, "load_core_context"):
            ...
            with patch(
                ...
                guide = agent.get_guide_for_task(
                    "custom_task", guides=["custom-guide", "another-guide"]
                )
                ...
                expected_calls = [call("guide_custom-guide.md"), call("guide_another-guide.md")]
...
def test_load_core_context_from_core_bank(
    mock_memory_instance, agent_root, spirit_file, core_files, temp_dir
):
    """Test loading core context from a core bank at MEMORY_BANKS_ROOT."""
    with patch("legacy_logseq.cogni_agents.base.FileMemoryBank") as mock_core_bank_cls:
        ...
        with patch.object(DummyAgent, "load_spirit"):
            ...
            expected_fallback_paths = {
                ...
                "guide_cogni-core-spirit.md": temp_dir
            ...
            expected_files = ["CHARTER.md", "MANIFESTO.md", "LICENSE.md", "README.md"]

            for file_name in expected_files:
                mock_core_bank.load_or_seed_file.assert_any_call(
                    file_name=file_name, fallback_path=expected_fallback_paths[file_name]
                )
...
def test_load_spirit_not_found_in_core_bank(
    mock_load_seed_file, agent_root, spirit_file, mock_memory_instance, temp_dir
):
