...
from legacy_logseq.memory.schema import MemoryBlock, QueryResult
from legacy_logseq.memory.storage import CombinedStorage, ChromaStorage, ArchiveStorage
...
class CogniMemoryClient:
    ...
    def __init__(self, chroma_path: str, archive_path: str, collection_name: str = "cogni-memory"):
        """
        ...
        self.storage = CombinedStorage(vector_db_dir=chroma_path, archive_dir=archive_path)
    ...
    def query(
        ...
        return QueryResult(query_text=query_text, blocks=blocks, total_results=len(blocks))
    ...
    def scan_logseq(
        self, logseq_dir: str, tag_filter: Optional[Union[List[str], Set[str], str]] = None
    ) -> List[MemoryBlock]:
        """
    ...
    def get_page(
        self, filepath: str, extract_frontmatter: bool = False
    ...
    def write_page(
        self, filepath: str, content: str, append: bool = False, frontmatter: Optional[Dict] = None
    ) -> str:
        """
    ...
    def index_from_logseq(
        ...
        try:
            ...
            for block in tqdm(raw_blocks, desc="Processing blocks", disable=not verbose):
                ...
                metadata = {"tags": tags_str, "source": block["source_file"]}
            ...
            if (
                embed_model == "mock"
                and hasattr(embed_fn, "__self__")
                and isinstance(embed_fn.__self__, MagicMock)
            ):
            ...
            if len(all_embeddings) != num_blocks:
                logger.warning(
                    f"Embedding count mismatch: {len(all_embeddings)} embeddings for {num_blocks} texts"
                )
            ...
            self.storage.chroma.collection.add(
                ids=all_ids, embeddings=all_embeddings, documents=all_texts, metadatas=all_metadatas
            )
