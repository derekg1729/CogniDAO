...
from legacy_logseq.memory.schema import MemoryBlock, ArchiveIndex, IndexMetadata
...
class ChromaStorage:
    ...
    def add_blocks(self, blocks: List[Union[MemoryBlock, Dict]]):
        ...
        for block in memory_blocks:
            metadata = {"tags": ", ".join(block.tags), "source": block.source_file}
            if block.source_uri:
...
class ArchiveStorage:
    ...
    def _update_index(self):
        ...
        # Create index data
        index_metadata = IndexMetadata(updated_at=datetime.now())
        ...
        # Create index
        index = ArchiveIndex(metadata=index_metadata, blocks=blocks_dict)
...
class CombinedStorage:
    ...
    def query(
        ...
        if filter_tags:
            ...
            combined_documents = hot_results["documents"][0] + [
                block["text"] for block in archived_results
            ]
            combined_metadatas = hot_results["metadatas"][0] + [
                {
                    "tags": ", ".join(block["tags"])
                    if isinstance(block["tags"], list)
                    else block["tags"],
