...
def memory_tool(
    ...
    chroma_path: str = "legacy_logseq/memory/chroma",
    archive_path: str = "legacy_logseq/memory/archive",
    ...
    if operation == "query":
        ...
        if not blocks_to_save:
            return {"operation": "save", "error": "No blocks provided to save", "success": False}
        ...
        try:
            ...
            return {"operation": "save", "error": f"Save failed: {str(e)}", "success": False}
        ...
        try:
            ...
            return {"operation": "archive", "error": f"Archive failed: {str(e)}", "success": False}
        ...
        try:
            ...
            if block:
                return {"operation": "get", "success": True, "block": block.model_dump()}
            else:
                return {
                    "operation": "get",
                    "success": False,
            ...
            return {"operation": "get", "success": False, "error": f"Get failed: {str(e)}"}

    elif operation == "count":
        # Count blocks in storage
        try:
            counts = client.count_blocks()
            return {"operation": "count", "success": True, "counts": counts}
        except Exception as e:
            return {"operation": "count", "success": False, "error": f"Count failed: {str(e)}"}
