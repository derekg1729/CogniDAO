    ...
    def test_e2e_pipeline(self, test_dirs):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for doc_path in found_docs:
                ...
                with (
                    open(doc_path, "r", encoding="utf-8") as src,
                    open(copied_path, "w", encoding="utf-8") as dst,
                ):
                    content = src.read()
                    dst.write(content)
            ...
            for doc in copied_files:
                with open(doc, "r", encoding="utf-8") as f:
                    ...
                    print(
                        f"File {os.path.basename(doc)}: {doc_lines} lines, {doc_content} content lines"
                    )
            ...
            assert success_rate >= 0.5, (
                f"Only {successful_queries}/{len(queries)} queries returned results"
            )

            print("\n=== ✅ E2E PIPELINE TEST COMPLETED SUCCESSFULLY ===")
            print(
                f"Indexed {num_indexed} blocks, {successful_queries}/{len(queries)} queries successful"
            )
    ...
    def test_direct_markdown_extraction(self, test_dirs):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                open(direct_file, "w", encoding="utf-8") as dst,
            ):
                content = src.read()
                dst.write(content)
            ...
            with open(direct_file, "r", encoding="utf-8") as f:
                ...
                print(
                    f"File: {total_lines} lines, {bullet_lines} bullet points, {content_lines} content lines"
                )
            ...
            bullet_capture = (
                (direct_indexed / max(1, bullet_lines)) * 100 if bullet_lines > 0 else 0
            )
