...
"""
            ...
            assert num_indexed == non_empty_bullets, (
                f"Expected {non_empty_bullets} blocks, got {num_indexed}"
            )
...
""")
            ...
            for line in lines:
                ...
                if "Important" in line:
                    ...
                    if any(
                        "Important" in line_text
                        for line_text in converted_lines[-5:]
                        if line_text.strip()
                    ):
                        converted_lines.append(f"- {line.strip()} #important")
                    elif any(
                        "Reference" in line_text
                        for line_text in converted_lines[-5:]
                        if line_text.strip()
                    ):
                        converted_lines.append(f"- {line.strip()} #reference")
            ...
            print(
                f"File contains: {important_blocks} important blocks, {reference_blocks} reference blocks, {note_blocks} note blocks"
            )
            ...
            num_indexed = client.index_from_logseq(
                logseq_dir=logseq_dir, tag_filter="#important", verbose=True
            )

            print(f"Indexed {num_indexed} important blocks")

            # Assert correct number of blocks indexed
            assert num_indexed == important_blocks, (
                f"Expected {important_blocks} important blocks, got {num_indexed}"
            )
